...
def summarize(state, runs, day):
    outcomes = {key: [] for key in ["previewed", "preview_partial", "published", "skipped"]}
    discovered, pending = [], []
    for item in state.get("items", {}).values():
        ...
        if item.get("status") in {"discovered", "prepared", "blocked", "preview_partial", "review", "submitting", "publish_uncertain"}:
            pending.append({"id": item["status_id"], "status": item["status"], "reason": item.get("last_result", {}).get("reason", "等待处理"), "block_kind": item.get("last_result", {}).get("block_kind"), "resume_condition": item.get("last_result", {}).get("resume_condition"), "retry_after": item.get("last_result", {}).get("retry_after")})
...
def markdown(result):
    ...
    lines = [f'# X 博主运营日报 · {result["date"]}', '',
             ...
             f'- 完整预览（新版验收）：{sum(v.get("preview_contract_version") == 1 for v in outcome["previewed"])} 条',
             f'- 历史预览（未按新版验收）：{sum(v.get("preview_contract_version") != 1 for v in outcome["previewed"])} 条',
             f'- 文字已展示、媒体待处理：{len(outcome.get("preview_partial", []))} 条（与完整预览可为同帖不同阶段，不相加）',
    ...
    for label, status in [('对话预览','previewed'), ('文字预览／媒体待处理','preview_partial'), ('实际发布','published'), ('跳过明细','skipped')]:
        lines += [f'## {label}', '']
        for item in outcome.get(status, []):
        ...
        lines.append('')
    lines += ['## 待处理（生成时快照）', '']
    lines += [f'- {v["id"]} · {v["status"]}：{v["reason"]}；阻塞类型：{v.get("block_kind") or "未记录"}；恢复条件：{v.get("resume_condition") or v.get("retry_after") or "未记录"}' for v in result['pending_at_report_time']] or ['无待处理记录。']
    lines += ['', '## 用户反馈与规则调整', '']
...
def main():
    ...
    print(json.dumps({'report': str(args.output.resolve()), 'date': result['date'], 'previews':len(result['outcomes']['previewed']), 'partial_previews':len(result['outcomes']['preview_partial']), 'published':len(result['outcomes']['published'])}, ensure_ascii=False))
